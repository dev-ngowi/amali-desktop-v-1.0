...
class SalesDetailedReport(QWidget):
    ...
    def init_ui(self):
        ...
        # Title
        title_label = QLabel("Sales Detailed Report")
        ...
        self.generate_report_button.clicked.connect(self.generate_sales_detailed)
        ...
        # Report Table
        self.detailed_table_view = QTableView()
        self.detailed_table_view.setEditTriggers(QAbstractItemView.NoEditTriggers)
        layout.addWidget(self.detailed_table_view)

        # Download Option
        download_layout = QHBoxLayout()
        download_layout.addStretch(1)
        self.download_button = QPushButton("Download as PDF")
        self.download_button.clicked.connect(self.download_sales_detailed_pdf)
    ...
    def generate_sales_detailed(self):
        ...
        sales_data = self.report_manager.get_sales_detailed_data(
        ...
        for order in sales_data:
            for item in order["items"]:
                row = [
                    QStandardItem(order["order_number"]),
                    ...
                    QStandardItem(f"{order['discount']:.2f}"),
                    QStandardItem(f"{order['ground_total']:.2f}"),
                ]
                model.appendRow(row)

        self.detailed_table_view.setModel(model)
        self.detailed_table_view.horizontalHeader().setStretchLastSection(True)
        self.detailed_table_view.resizeColumnsToContents()

    def download_sales_detailed_pdf(self):
        ...
        model = self.detailed_table_view.model()
        if model is None:
            QMessageBox.warning(self, "No Data", "No sales detailed data to download.")
            return

        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save Sales Detailed Report as PDF", "", "PDF (*.pdf)"
        )
        ...
        if file_path:
            ...
            try:
                ...
                story.append(Paragraph("<b>Sales Detailed Report</b>", title_style))
                start_date = self.start_date_edit.date().toString("yyyy-MM-dd")
                end_date = self.end_date_edit.date().toString("yyyy-MM-dd")
                ...
                QMessageBox.information(
                    self, "Success", f"Sales detailed report downloaded to: {file_path}"
                )
