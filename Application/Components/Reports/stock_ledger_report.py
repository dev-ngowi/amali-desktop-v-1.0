...
from PyQt5.QtWidgets import (
    ...
    QPushButton, QTableView, QAbstractItemView, QFileDialog, QMessageBox, QComboBox
)
...
class StockLedgerReport(QWidget):
    ...
    def init_ui(self):
        ...
        # Title
        title_label = QLabel("Stock Ledger Report")
        ...
        self.generate_report_button.clicked.connect(self.generate_stock_ledger)
        ...
        # Report Table
        self.ledger_table_view = QTableView()
        self.ledger_table_view.setEditTriggers(QAbstractItemView.NoEditTriggers)
        layout.addWidget(self.ledger_table_view)

        # Download Option
        download_layout = QHBoxLayout()
        download_layout.addStretch(1)
        self.download_button = QPushButton("Download as PDF")
        self.download_button.clicked.connect(self.download_stock_ledger_pdf)
        download_layout.addWidget(self.download_button)
        layout.addLayout(download_layout)

        self.setLayout(layout)

    def generate_stock_ledger(self):
        ...
        stock_ledger_data = self.report_manager.get_stock_ledger_data(start_date, end_date, selected_store_id)
        ...
        for entry in stock_ledger_data:
            row = [
                ...
                QStandardItem(entry["reference"]),
                QStandardItem(f"{entry['inflow']:.2f}"),
                QStandardItem(f"{entry['outflow']:.2f}"),
                QStandardItem(f"{entry['balance']:.2f}"),
            ]
            model.appendRow(row)

        self.ledger_table_view.setModel(model)
        self.ledger_table_view.horizontalHeader().setStretchLastSection(True)
        self.ledger_table_view.resizeColumnsToContents()

    def download_stock_ledger_pdf(self):
        ...
        model = self.ledger_table_view.model()
        if model is None:
            QMessageBox.warning(self, "No Data", "No stock ledger data to download.")
            return

        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save Stock Ledger Report as PDF", "", "PDF (*.pdf)"
        )
            ...
            try:
                ...
                if company_details and company_details[0]:
                    ...
                    if company_info["phone"]:
                        story.append(Paragraph(f"Phone: {company_info['phone']}", normal_style))
                    story.append(Spacer(1, 0.25 * inch))

                story.append(Paragraph("<b>Stock Ledger Report</b>", title_style))
                start_date = self.start_date_edit.date().toString("yyyy-MM-dd")
                end_date = self.end_date_edit.date().toString("yyyy-MM-dd")
                ...
                for row in range(model.rowCount()):
                    ...
                    for col in range(model.columnCount()):
                        ...
                        # Wrap text for the "Item Name" column (col 0)
                        if col == 0:
                ...
                col_widths = [2.0 * inch, 1.2 * inch, 1.2 * inch, 0.8 * inch, 0.8 * inch, 0.8 * inch]
                ...
                QMessageBox.information(
                    self, "Success", f"Stock ledger report downloaded to: {file_path}"
                )
